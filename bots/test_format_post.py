...
class TestFormatPost(unittest.TestCase):
    ...
    def test_shorten_status(self):
        ...
        # Status is <= limit
        self.assertLessEqual(50, len(format_post.shorten_status(status, closing, limit)))
        # Closing is attached (if over limit)
        self.assertEqual(closing, format_post.shorten_status(status, closing, limit)[-7:])
        # Closing is NOT attached (if under limit)
        self.assertNotEqual(closing, format_post.shorten_status(status[:50], closing, limit)[-7:])

    def test_clean_description(self):
        # Remove HTML tags/codes & spoilers
        self.assertTrue('</p>' not in format_post.clean_description('<p>a tag</p>'))
        self.assertTrue('&quot;' not in format_post.clean_description('he then &quot; went swimming'))
        self.assertTrue('!~' not in format_post.clean_description('~! spoiler!! !~'))
